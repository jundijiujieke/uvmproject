 ...
 * February 2005
 *
 * dct4.c - optrigtabs.ctimized DCT-IV
 **************************************************************************************/
#include <stdint.h>
#include <stdlib.h>
#include <stdio.h>
...
void DCT4_SV(const datasize *before, datasize *after, const int memsize,const int tab){

  		
	int mem[memsize];
	
	memcpy(mem,before,memsize*4);//copy datasize of data, n(bytes),32*memsize(bit), 4*memsize(bytes)

	fp1 = fopen("before.txt","w");

    for(i=0;i<memsize;i++)
    {
        fprintf(fp1,"%08x\n",mem[i]);//output 8 char, x means 16(4bits),4*8(32bits)
    }
...
/*int main(){
	...
	DCT4_SV(&mem_array,&mem_after,128,0);
...
char str[] = "This is a test";
	...
	fp1 = fopen("before.txt","w");
		...
		for (i=0; i<128; i++)
	{	
//		fwrite(mem_array+4*i,sizeof(mem_array),1,fp1);
		fprintf(fp1, "%08x\n", mem_array[i]);
	}	
	fclose(fp1);
//	DCT4(mem_array,mem_after,128,0)
	DCT4(0, mem_array, 2); 	//imdct64
